...
このトランスコーダーは、WebCodecsとMediaBunnyを基盤として最高のパフォーマンスを実現しています。主な最適化機能：

- **ハードウェアアクセラレーション**: GPUを自動活用してエンコード・デコードを高速化
- **パススルーモード**: ビットレートを「維持」に設定すると、超高速ストリームコピー
- **ゼロコピー転送**: Worker間で効率的なメモリ管理
- **スマートコーデック選択**: 速度優先ならH.264、圧縮優先ならAV1

**📖 詳細なパフォーマンスガイドとコーデック選択のコツは [パフォーマンスガイド](performance-guide.md) を参照**

### 速度の目安
- **パススルーモード**: どのファイルも10-30秒（コンテナ再構築のみ）
- **H.264トランスコード**: 1080pでリアルタイムの2-4倍速（ハードウェアアクセラレーション時）
- **H.265/AV1**: 圧縮率は高いが処理は遅い（詳細はガイド参照）
